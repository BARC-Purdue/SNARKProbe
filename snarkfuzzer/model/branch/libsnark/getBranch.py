...
class ProgBranch(object):
    ...
    def checkbptvalid(self):
        ...
        exit = subprocess.run(["gdb", "-q", "-x", self.prebptgdb, self.source], stdout = subprocess.PIPE, stderr = subprocess.PIPE)
    ...
    def findbranch(self):
        ...
        exit = subprocess.run(["gdb", "-q", "-x", self.structgdb, self.source], stdout = subprocess.PIPE, stderr = subprocess.PIPE)
        ...
        for infoline in content:
            if (infoline.startswith("Breakpoint ")):
