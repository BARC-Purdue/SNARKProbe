...
class FF(object):
    ...
    def inverse(self):
        inv = utils.prime_field_inv(self.num, bn128.curve_order)
        return FF(inv)
...
class G1(object):
    def __init__(self, x, y, z = 1):
        ...
        self.g1 = (bn128.FQ(int(x)), bn128.FQ(int(y)))
    ...
    def __mul__(self, other):
        if (isinstance(other, int)):
            g1 = bn128.multiply(self.g1, other)
        elif (isinstance(other, FF)):
            g1 = bn128.multiply(self.g1, other.num)
        else:
            raise Exception
...
class G2(object):
    def __init__(self, x, y, z = (1, 0)):
        ...
        self.g2 = (bn128.FQ2(list(x)), bn128.FQ2(list(y)))
    ...
    def __mul__(self, other):
        if (isinstance(other, int)):
            g2 = bn128.multiply(self.g2, other)
        elif (isinstance(other, FF)):
            g2 = bn128.multiply(self.g2, other.num)
        else:
            raise Exception
...
def pairing(point1, point2):
    if (isinstance(point1, G1) and isinstance(point2, G2)):
        pair = bn128.pairing(point2.g2, point1.g1)
    elif (isinstance(point1, G2) and isinstance(point2, G1)):
        pair = bn128.pairing(point1.g2, point2.g1)
    else:
        raise Exception
