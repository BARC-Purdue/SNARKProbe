...
def randomNormal(low, upp, mean):
    std = mean / 2.5

    X = get_truncated_normal(mean = mean, sd = std, low = low, upp = upp)
...
class AtherisMutation(object):
    ...
    def readSeed(self):
        ...
        self.num_pubinput = randomNormal(0, self.num_variables - 1, info[2])
        self.witness = info[3]
    ...
    def writeMatrix(self, fuzzbytelist, i = 0):
        ...
        dst_name = "mutation_" + srcbase.split(".")[0] + "_invalid_" + str(i) + ".txt"
        ...
        f = open(dst, "w")
        f.write(str([self.num_variables, self.num_constraint, self.num_pubinput, self.witness, r1csmatrix]))
        f.close()
